...
def test_static_struct_suffix() -> i32:
    ...
    # (10+20)=30 and (1.5+2.5)=4 -> 304
    return (p1.x + p1.y) * 10 + i32(p2.x + p2.y)
...
def test_dynamic_struct_suffix() -> i32:
    ...
    # (15+35)=50 and (2.0+3.0)=5 -> 505
    return (result.x + result.y) * 10 + i32(result2.x + result2.y)
...
def test_dynamic_union_suffix() -> i32:
    ...
    if result1 != 9876543210:
        return -1
    # f32(3.14) * 100 rounds to 314
    return i32(result2 * 100)
...
@compile
def main() -> i32:
    if test_static_struct_suffix() != 304:
        return 1
    if test_dynamic_struct_suffix() != 505:
        return 1
    if test_static_union_suffix() != 42:
        return 1
    if test_dynamic_union_suffix() != 314:
        return 1
    if test_static_enum_suffix() != 0:
        return 1
    if test_dynamic_enum_suffix() != 0:
        return 1
    return 0
...
class TestCompositeSuffix(unittest.TestCase):
    ...
    def test_static_struct_suffix(self):
        self.assertEqual(test_static_struct_suffix(), 304)

    def test_dynamic_struct_suffix(self):
        self.assertEqual(test_dynamic_struct_suffix(), 505)

    def test_static_union_suffix(self):
        self.assertEqual(test_static_union_suffix(), 42)

    def test_dynamic_union_suffix(self):
        self.assertEqual(test_dynamic_union_suffix(), 314)

    def test_static_enum_suffix(self):
        self.assertEqual(test_static_enum_suffix(), 0)

    def test_dynamic_enum_suffix(self):
        self.assertEqual(test_dynamic_enum_suffix(), 0)
