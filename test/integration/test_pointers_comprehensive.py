...
@compile
def test_ptr_ptr_sub() -> i64:
    """Test ptr - ptr returns element count"""
    arr: array[i32, 10] = array[i32, 10]()
    p1: ptr[i32] = arr
    p2: ptr[i32] = arr + 5
    return p2 - p1  # Should be 5


@compile
def test_arr_ptr_sub() -> i64:
    """Test arr - ptr returns element count"""
    arr: array[i32, 10] = array[i32, 10]()
    p: ptr[i32] = arr + 5
    return arr - p  # Should be -5


@compile
def test_ptr_arr_sub() -> i64:
    """Test ptr - arr returns element count"""
    arr: array[i32, 10] = array[i32, 10]()
    p: ptr[i32] = arr + 3
    return p - arr  # Should be 3
...
@compile
def test_struct_ptr_sub() -> i64:
    """Test ptr - ptr with struct type returns element count"""
    arr: array[struct[i32, i32, i32], 5] = array[struct[i32, i32, i32], 5]()
    p1: ptr[struct[i32, i32, i32]] = arr
    p2: ptr[struct[i32, i32, i32]] = arr + 3
    return p2 - p1  # Should be 3 (not 3 * sizeof(struct))


@compile
def test_struct_arr_ptr_sub() -> i64:
    """Test arr - ptr and ptr - arr with struct type"""
    arr: array[struct[i32, i32], 10] = array[struct[i32, i32], 10]()
    p: ptr[struct[i32, i32]] = arr + 4
    diff1: i64 = arr - p  # -4
    diff2: i64 = p - arr  # 4
...
class TestPointerArithmetic(unittest.TestCase):
    ...
    def test_ptr_ptr_sub(self):
        self.assertEqual(test_ptr_ptr_sub(), 5)
    
    def test_arr_ptr_sub(self):
        self.assertEqual(test_arr_ptr_sub(), -5)
    
    def test_ptr_arr_sub(self):
        self.assertEqual(test_ptr_arr_sub(), 3)
...
class TestPointerWithStruct(unittest.TestCase):
    ...
    def test_struct_ptr_sub(self):
        self.assertEqual(test_struct_ptr_sub(), 3)
    
    def test_struct_arr_ptr_sub(self):
        self.assertEqual(test_struct_arr_ptr_sub(), 0)
