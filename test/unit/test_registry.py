...
class TestUnifiedRegistry(unittest.TestCase):
    ...
    def test_variable_declare_via_scope_manager(self):
        """Test variable declaration and lookup through ScopeManager"""
        from pythoc.valueref import ValueRef
        from pythoc.builtin_entities import i32
        from pythoc.scope_manager import ScopeManager, ScopeType
        ...
        sm.declare_variable(var_info)

        retrieved = sm.lookup_variable("x")
        self.assertEqual(retrieved, var_info)

    def test_variable_scope_via_scope_manager(self):
        """Test variable scope management through ScopeManager"""
        from pythoc.valueref import ValueRef
        from pythoc.builtin_entities import i32
        from pythoc.scope_manager import ScopeManager, ScopeType
        ...
        sm.declare_variable(var_info1, allow_shadow=True)
        sm.enter_scope(ScopeType.IF)
        ...
        sm.declare_variable(var_info2, allow_shadow=True)

        # Should get the inner scope variable
        retrieved = sm.lookup_variable("x")
        self.assertEqual(retrieved, var_info2)

        # Exit inner scope (use _var_registry directly to avoid needing ControlFlowBuilder)
        sm._scopes.pop()
        sm._var_registry.exit_scope()
