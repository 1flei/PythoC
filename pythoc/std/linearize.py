...
def linearize(acquire_func, release_func, struct_name=None):
    ...
    # If we are inside an effect override context, eagerly create effect-specialized
    # versions of acquire/release so the wrapper has concrete, compiled targets.
    # This avoids relying on transitive specialization through captured callables.
    from ..effect import get_current_effect_suffix, capture_effect_context
    ctx_effect_suffix = get_current_effect_suffix()
    if ctx_effect_suffix:
        effect_overrides = capture_effect_context()
        if hasattr(acquire_func, 'get_effect_specialized'):
            acquire_func = acquire_func.get_effect_specialized(ctx_effect_suffix, effect_overrides)
        if hasattr(release_func, 'get_effect_specialized'):
            release_func = release_func.get_effect_specialized(ctx_effect_suffix, effect_overrides)
