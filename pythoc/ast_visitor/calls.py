...
class CallsMixin:
    ...
    def visit_Call(self, node: ast.Call):
        ...
        The visitor evaluates the callee expression and positional arguments,
        then normalizes typed ``*args`` / ``**kwargs`` into literal carriers
        before delegating to ``ValueRefDispatcher.handle_call()``.
        """
        ...
        for arg in node.args:
            if isinstance(arg, ast.Starred):
                ...
                positional_args.extend(expanded_args)
            else:
                positional_args.append(self.visit_rvalue_expression(arg))

        from ..call_normalization import normalize_ast_call_args
        args = normalize_ast_call_args(
            func_ref,
            positional_args,
            node.keywords,
            self.visit_rvalue_expression,
            node,
        )
