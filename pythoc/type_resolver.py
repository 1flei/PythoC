...
class TypeResolver:
    ...
    def _evaluate_type_expression(self, node):
        ...
        if isinstance(node, ast.Subscript):
            ...
            elif isinstance(base, ValueRef) and base.type_hint and hasattr(base.type_hint, 'handle_subscript'):
            ...
            if subscriptable:
                ...
                try:
                    ...
                    raise TypeError(f"Cannot subscript {base_value} with {key_value}: {e}")
    ...
    def annotation_to_llvm_type(self, annotation) -> ir.Type:
        ...
        if isinstance(builtin_type, type) and hasattr(builtin_type, '_is_struct') and builtin_type._is_struct:
        ...
        if hasattr(builtin_type, 'get_llvm_type'):
            ...
            raise TypeError(f"TypeResolver: cannot extract LLVM type from {builtin_type} (type: {type(builtin_type)})")
